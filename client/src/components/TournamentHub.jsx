...
export default function TournamentHub({ tournament, onStart, onPlayMatch, onExit }) {
  ...
  /**
   * UNA sola llamada para las DOS hojas (vestíbulo y cuadro): nunca están
   * montadas a la vez, comparten título y comparten salida. Va aquí arriba, por
   * encima de los tres `return` condicionales de abajo, porque un hook detrás
   * de un return es el «Rendered fewer hooks than expected» que este repo ya ha
   * pagado dos veces.
   */
  const { propsPanel, propsTitulo } = useModalA11y(onExit);
    ...
    return (
      <div className="modal-overlay animate-fade-in">
        <div className="modal-card glass-panel animate-scale-up bracket-modal-card modal-a11y" {...propsPanel} onClick={(e) => e.stopPropagation()}>
          <button type="button" className="profile-close" onClick={onExit} aria-label={t('tourney.exit')}>
          ...
          <div className="bracket-header">
            ...
            <div>
              <h3 className="bracket-title" {...propsTitulo}>{t('tourney.hubTitle')}</h3>
              <span className="bracket-subtitle">{t('tourney.hubSub', { reward })}</span>
            </div>
          </div>

          <button type="button" className="tourney-code-box" onClick={copyCode} title={t('tourney.copyCode')} aria-label={t('tourney.copyCode')}>
            <span className="tourney-code-label">
              {copied ? <><Check size={11} aria-hidden="true" /> {t('tourney.copied')}</> : <><Copy size={11} aria-hidden="true" /> {t('tourney.shareCode')}</>}
            </span>
            <span className="tourney-code">{code}</span>
          </button>

          <div className="tourney-players">
            <div className="profile-section-label"><Users size={14} aria-hidden="true" /> {t('tourney.players')} ({humans.length}/4)</div>
            {slots.map((s, i) => (
              <div key={i} className={`tourney-player-slot ${s.filled ? 'filled' : ''}`}>
                ...
                {s.filled && i === 0 && <Crown size={12} className="text-amber-400" aria-label={t('tourney.host')} />}
              </div>
            ))}
          ...
          <div className="tourney-cta">
            {isHost ? (
              <button type="button" className="btn-premium btn-primary tourney-cta-btn" onClick={onStart}>
                <Play size={18} aria-hidden="true" /> {t('tourney.start')}
              </button>
            ) : (
              <div className="tourney-resolving"><Hourglass size={16} aria-hidden="true" /> {t('tourney.waitHost')}</div>
            )}
          </div>
        </div>
      </div>
  ...
  const Player = ({ seedIdx, isWinner }) => (
    <div className={`bracket-player ${isWinner ? 'winner' : ''} ${seedIdx === youSeed ? 'is-human' : ''}`}>
      ...
      {isWinner && <Crown size={12} className="text-amber-400" aria-hidden="true" />}
    </div>
  );
      ...
      <div className="modal-card glass-panel animate-scale-up bracket-modal-card modal-a11y" {...propsPanel} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bracket-grid">
          <div className="bracket-column">
            <div className="bracket-phase-title"><Swords size={14} aria-hidden="true" /> {t('tourney.semis')}</div>
          ...
          <div className="bracket-column final-column">
            <div className="bracket-phase-title gold"><Crown size={14} aria-hidden="true" /> {t('tourney.final')}</div>
        ...
        <div className="tourney-cta">
          {status === 'active' && yourMatchRoomId && (
            <button type="button" className="btn-premium btn-primary tourney-cta-btn" onClick={onPlayMatch}>
              <Swords size={18} aria-hidden="true" /> {t('tourney.playMatch')}
            </button>
          )}
          {status === 'active' && !yourMatchRoomId && eliminated && (
            <div className="tourney-result">
              ...
              <button type="button" className="btn-premium btn-secondary tourney-cta-btn" onClick={onExit}>
                <LogOut size={16} aria-hidden="true" /> {t('tourney.exit')}
              </button>
            </div>
          )}
          {status === 'active' && !yourMatchRoomId && !eliminated && (
            <div className="tourney-resolving"><Loader2 size={16} className="voice-spin" aria-hidden="true" /> {t('tourney.waitOther')}</div>
          )}
